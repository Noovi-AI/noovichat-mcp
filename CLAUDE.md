...
**The only thing outside this autonomous loop is production release** — `pnpm version` + `pnpm publish --access public` + the tag push. Run `/pre-publish-audit` first (deploy window + clean tree + HEAD pushed + `pnpm check` + `pnpm build` + `dist/index.js` present); it needs human approval + the 5 golden rules and is gated by the root `pre-deploy-gate.sh` hook. The internal-docs update (step 8) IS part of the loop and closes it.
